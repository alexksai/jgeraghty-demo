...
export default async function Page({params}){
  ...
  return <main>
    <Header/>
    <section className="pageHero serviceHero" style={{'--hero':`url(${asset(s.image)})`}}>
      <div className="heroShade"/>
      <div className="pageHeroCopy"><span>{s.eyebrow}</span><h1>{s.title}</h1><i/><p>{s.subtitle}</p><div className="heroBtns"><Link href="/contact">Get a quote ↗</Link><a href="#overview">Explore service ↓</a></div></div>
    </section>
    <section className="content overview" id="overview"><div><span>01 / Overview</span><h2>{s.subtitle}</h2></div><div><p>{s.intro}</p></div></section>
